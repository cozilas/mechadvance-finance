...
import { invoices as api, clients as clientsApi } from '../api';
    ...
    <form onSubmit={handleSubmit(onSave)} className="space-y-5">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input {...register('due_date', { required: true })} type="date" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div>
...
export default function Invoices() {
  ...
  const [modal, setModal] = useState<'create' | Invoice | 'view' | null>(null);
  ...
  const columns = [
    ...
    { key: 'client',     header: 'Client',  render: (r: Invoice) => r.client?.name ?? '—' },
    { key: 'issue_date', header: 'Issued',  render: (r: Invoice) => r.issue_date?.slice(0, 10) },
    { key: 'due_date',   header: 'Due',     render: (r: Invoice) => r.due_date?.slice(0, 10) },
    { key: 'total',      header: 'Total',   render: (r: Invoice) => fmt(r.total) },
    { key: 'status',     header: 'Status',  render: (r: Invoice) => <StatusBadge status={r.status} /> },
    {
      ...
      render: (r: Invoice) => (
        <div className="flex gap-2 justify-end" onClick={e => e.stopPropagation()}>
          ...
          <button onClick={() => confirm('Delete invoice?') && remove.mutate(r.id)} className="text-gray-400 hover:text-red-600"><Trash2 size={14} /></button>
        </div>
      ),
    },
  ];
    ...
    <div>
      ...
      {modal === 'create' && (
        <Modal title="New Invoice" onClose={() => setModal(null)} wide>
          <InvoiceForm onSave={(d) => create.mutate(d)} onClose={() => setModal(null)} />
        </Modal>
      )}

      {modal && modal !== 'create' && modal !== 'view' && (
        <Modal title="Edit Invoice" onClose={() => setModal(null)} wide>
          <InvoiceForm
            initial={modal as Invoice}
            onSave={(d) => update.mutate({ ...d, id: (modal as Invoice).id })}
      ...
      {modal === 'view' && detail && (
        <Modal title={`Invoice ${detail.number}`} onClose={() => { setModal(null); setViewing(null); }} wide>
          <div className="space-y-4 text-sm">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><span className="text-gray-500">Due:</span> {detail.due_date?.slice(0, 10)}</div>
            </div>
              ...
              <tbody>
                {detail.items?.map((item, i) => (
                  <tr key={i} className="border-b border-gray-50">
                    ...
                    <td className="text-right py-2">{fmt(item.total)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
